...
def get_ftle(ftLe_backward, ftLe_forward, ftLe_intersect, grad_sig, mesh, ftle_f, tstep):
    ...
    e_min = {ui:utilities.CG1Function(e_min_DG0[i], mesh, method=_krylov_solver, name="e_min") for i, ui in enumerate(components)} #project to CG1
    e_min['0']()
    e_min['1']()
    e_min['2']()
    _e_min = as_vector([e_min[ui] for ui in components])
    #get the minima
    lcs = utilities.CG1Function(ufl.dot(_grad_sig, _e_min), mesh, method=_krylov_solver, name="lcs") #scalar-valued    
    lcs()
    if MPI.rank(MPI.comm_world) == 0:
