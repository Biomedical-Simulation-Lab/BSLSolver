...
__editor__ = "2022 Anna Haley"
__date__ = "2018-04-21"
__copyright__ = "Copyright (C) 2018 U of T"
...
def read_mesh_info(mesh_info_path, key):
    ...
    lines = buf.split('\n')
    ids = []
    idfr = [] #flowrates from inlets and outlets calculated from area ratio given
    ida = [] #area
    idr = [] #radius
    fcs = [] #waveform
...
def get_period_from_fcs(fcs):
    ...
    for i,f in enumerate(fcs):
        ...
        if os.path.exists( os.path.join(os.path.expandvars('$SLURM_SUBMIT_DIR/data'), fcs_i_filename) ):
            fcs_ifname = os.path.join(os.path.expandvars('$SLURM_SUBMIT_DIR/data'), fcs_i_filename)
        else:
...
def get_period_from_info(mesh_name):
    id_in, Q_means, _, fcs = read_mesh_info(os.path.expandvars('$SLURM_SUBMIT_DIR/data/')+mesh_name+'.info', '<INLETS>')
