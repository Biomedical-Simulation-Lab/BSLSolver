...
def temporal_hook(u_, p_, p, q_, V, mesh, tstep, compute_flux,
                  ...
                  cpu_time, final_time, timesteps, not_zero_pressure_outlets,ftle_f, forward_ftLe, backward_ftLe, **NS_namespace):
    ...
    if (current_cycle > 0) and (current_cycle <= total_cycles-1): #
        if tstep % store_data == 0:	
            ...
            if NS_parameters['save_ftle']:
                FTLE.get_ftle(forward_ftLe, backward_ftLe, ftle_f, tstep)
            #if mpi_rank == 0:
